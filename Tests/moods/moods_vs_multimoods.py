...
for r in MOODS.search(seq, pssm_list, threshlist, absolute_threshold=True, both_strands=True):
...
for i in range(0,len(pssm_list)):
  ...
  for r in MOODS.search(seq, [pssm], threshold, absolute_threshold=True, both_strands=True):
    for (position, score) in r:
      if(position >= 0):
        p1 = position
        strand = "+"
